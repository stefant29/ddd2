...
import com.dignitas.ddd2.repository.ARepository;
...
/**
 * REST controller for managing {@link com.dignitas.ddd2.domain.A}.
 */
@RestController
...
public class AResource {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<A> updateA(@PathVariable(value = "id", required = false) final String id, @RequestBody A a)
        ...
        A result = aRepository.save(a);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, a.getId())).body(result);
    }
    ...
    public ResponseEntity<A> partialUpdateA(@PathVariable(value = "id", required = false) final String id, @RequestBody A a)
        throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, a.getId()));
    }
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteA(@PathVariable("id") String id) {
