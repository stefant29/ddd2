...
/**
 * REST controller for managing {@link com.dignitas.ddd2.domain.B}.
 */
@RestController
...
public class BResource {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<B> updateB(@PathVariable(value = "id", required = false) final String id, @RequestBody B b)
        ...
        B result = bRepository.save(b);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, b.getId())).body(result);
    }
    ...
    public ResponseEntity<B> partialUpdateB(@PathVariable(value = "id", required = false) final String id, @RequestBody B b)
        throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, b.getId()));
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<B> getB(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteB(@PathVariable("id") String id) {
