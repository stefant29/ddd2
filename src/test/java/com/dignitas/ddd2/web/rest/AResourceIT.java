...
class AResourceIT {
    ...
    @Mock
    private ARepository aRepositoryMock;
    ...
    void getAllASWithEagerRelationshipsIsEnabled() throws Exception {
        when(aRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(aRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllASWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(aRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(aRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
